...
func (q *QuadStore) AddQuad(x *graph.Quad) {
...
func (q *QuadStore) AddQuadSet(xset []*graph.Quad) {
...
func (q *QuadStore) RemoveQuad(x *graph.Quad) {
	if x.Provenance != "" {
...
func (q *QuadStore) Quad(tid graph.Value) (qd *graph.Quad) {
	ok := false
	gotAll := true
	qd = &graph.Quad{}
	qv := tid.(QuadValue)
...
func (qs *QuadStore) QuadIterator(dir graph.Direction, val graph.Value) graph.Iterator {
...
func (q *QuadStore) QuadDirection(quad_id graph.Value, dir graph.Direction) graph.Value {
